...
class ShaderProgram {
  ...
  constructor(shaders: Array<Shader>) {
    ...
    this.unifSpecular   = gl.getUniformLocation(this.prog, "u_Specular");
    ...
    this.unifFog        = gl.getUniformLocation(this.prog, "u_FogColor");
  }
  ...
  setGeometrySpecular(color: vec4) {
    this.use();
    if (this.unifSpecular !== -1) {
      gl.uniform4fv(this.unifSpecular, color);
    }
  }
  ...
  setSunIntensity(intensity: number) {
    ...
    if (this.unifIntensity !== -1) {
      gl.uniform1f(this.unifIntensity, 10000.0/intensity);
    }
  }

  setFogColor(color: vec4) {
    this.use();
    if (this.unifFog !== -1) {
      gl.uniform4fv(this.unifFog, color);
    }
  }
